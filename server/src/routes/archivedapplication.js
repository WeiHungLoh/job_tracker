...
router.delete('/delete/:archivedJobId', async (req, res) => {
    // Since assignment object ID has been passed to param, retrieve it
    const { archivedJobId } = req.params
    const userId = req.user.id

    try {
        await deleteArchivedJobApplication(archivedJobId, userId)
...
router.delete('/unarchived/:archivedJobId', async (req, res) => {
    const { archivedJobId } = req.params
    const userId = req.user.id
