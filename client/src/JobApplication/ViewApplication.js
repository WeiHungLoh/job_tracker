...
const ViewApplication = () => {
    ...
    const { data: applications, refetch } = useFetchData(`${process.env.REACT_APP_API_URL}/job-applications`)
    const [jobStatuses, setJobStatuses] = useState({})
    const { data: interviews } = useFetchData(`${process.env.REACT_APP_API_URL}/job-interviews`)
    ...
    const handleDelete = async (applicationId) => {
        try {
            ...
            if (confirmed) {
                await fetch(`${process.env.REACT_APP_API_URL}/job-applications/${applicationId}`, {
                    method: 'DELETE',
                    credentials: 'include',
                    headers: { 'Content-type': 'application/json' },
    ...
    const handleDeleteAll = async () => {
        try {
            ...
            if (confirmed) {
                await fetch(`${process.env.REACT_APP_API_URL}/job-applications`,
                    {
                        method: 'DELETE',
                        credentials: 'include'
                    }
    ...
    const handleArchive = async (jobId) => {
        try {
            await fetch(`${process.env.REACT_APP_API_URL}/archived-job-applications/${jobId}`,
                {
                    method: 'POST',
                    credentials: 'include',
                    headers: { 'Content-Type': 'application/json' },
